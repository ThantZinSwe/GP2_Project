        ...
        <div class="card">
          ...
          <div class="card-body">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                @foreach (\App\Models\Role::get() as $role_list)
                  <tr>
                    <td>{{$role_list->name}}</td>
                    <td class="d-flex border-bottom-0">
                        <a href="{{route('admin.role.edit',$role_list->id)}}" class="btn btn-warning btn-sm mr-3">Edit</a>
                        <form method="post" action="{{route('admin.role.delete',$role_list->id)}}" >
                        {{ csrf_field() }}
                                {{ method_field('DELETE') }}
